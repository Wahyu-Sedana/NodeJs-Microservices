...
const loginPengguna = async (req, res) => {
  ...
  try {
    const [login] = await db.query("SELECT id_user AS id_user, _nama_usaha AS nama_usaha, _email AS email FROM user WHERE _email= :email AND _password= :password", {email, password: pass})
...
const tambahPengguna = async (req, res) => {
  ...
  try {
    const [checkEmail] = await db.queryAll("SELECT id_user AS id_user, _nama_usaha AS nama_usaha, _email AS email FROM user WHERE _email= :email AND _nama_usaha= :nama_usaha", {email, nama_usaha})
